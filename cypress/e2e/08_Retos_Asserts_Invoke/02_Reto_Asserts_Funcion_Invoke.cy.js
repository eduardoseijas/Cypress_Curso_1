    ...
    it("Test - Assert_Reto-1", function(){
        ...
        cy.get("#answer").should("be.visible").then((r)=>{
            ...
            if(res2>50){
                ...
                b=b-10              //.invoke llama desde el atributo = attr, en este caso type y luego decimos que contiene ese campo
                cy.get("#number1").invoke("attr","type").should("contain","text").then(()=>{
                    cy.get("#number1").clear().type(a)
                    cy.get("#number1").invoke("attr","style","color:blue")
                })
                cy.wait(time)
                cy.get("#number2").should("be.enabled").clear().type(b)
                cy.get("#number2").invoke("attr","style","color:blue")
                cy.wait(time)
                cy.get("#calculate").should("have.value","Calculate").should("have.class","styled-click-button").click({force:true})
                cy.wait(time)
